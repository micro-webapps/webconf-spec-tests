...
handled_by_8080() {
    ...
    curl -L "$url" 2>/dev/null|grep "8080" >/dev/null
    if [ $? != 0 ]; then
...
handled_by_8080_ssl() {
    ...
    curl -k -L "$url" -v 2>&1|grep "8080" -B100|grep "SSL connection using" >/dev/null
    if [ $? != 0 ]; then
...
not_handled_by_8080() {
    ...
    curl -k -L "$url" 2>/dev/null|grep "8080" >/dev/null
    if [ $? == 0 ]; then
...
handled_by_8081() {
    ...
    curl -k -L "$url" 2>/dev/null|grep "8081" >/dev/null
    if [ $? != 0 ]; then
